...
import {shippingAddressByCustomer, ordersByCustomer, SingleCall} from '../helpers/apiCalls';
import CustomerOrders from '../components/CustomerOrders';
import CustomerShippingAddress from '../components/CustomerShippingAddresses';
import style from '../style/Profile.module.css';

const Profile = props => {
  const {location, auth, orders, shippingAddress, getOrders, getShipping, getCustomer } = props;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        await getOrders(customer.customer._id);
        await getShipping(customer.customer._id);
      } catch (error) {
        console.log(error)
      }           
    })();
          ...
          <div className="col-md-8">
            <div className={`tab-content ${style.profiletab}`} id="myTabContent">
              ...
              <div className="tab-pane fade" id="shipping" role="tabpanel" aria-labelledby="shipping-tab">
                <CustomerShippingAddress addresses={shippingList} />
              </div>
            </div>
          </div>
        </div>
